...
int main(int argc, char *argv[])
{
    ...
    fd_di_bypass_all = open(di_bypass_all_path, O_CREAT | O_RDWR | O_TRUNC, 0664);
    if (fd_di_bypass_all >= 0) {
        write(fd_di_bypass_all, "1", strlen("1"));
        close(fd_di_bypass_all);
    }
    player_init();
    streamsource_init();
    ...
    while ((!tmpneedexit) && (!PLAYER_THREAD_IS_STOPPED(player_get_state(pid)))) {
        if (!osd_is_blank) {
            ...
            fd_new_frame_count = open("/sys/module/amvideo/parameters/new_frame_count", O_RDONLY);
            if (fd_new_frame_count >= 0) {
                read(fd_new_frame_count, newframestring, sizeof(newframestring) - 1);
                close(fd_new_frame_count);
                fd_new_frame_count = -1;
            }
            if (strcmp(newframestring, newstring0) != 0) {
    ...
    fd_di_bypass_all = open(di_bypass_all_path, O_CREAT | O_RDWR | O_TRUNC, 0664);
    if (fd_di_bypass_all >= 0) {
        write(fd_di_bypass_all, "0", strlen("0"));
        close(fd_di_bypass_all);
    }
    return 0;
}
