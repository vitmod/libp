...
static int get_spu_cmd(AML_SPUVAR *sub_frame)
{
    ...
    if(sub_frame->cmd_offset >= sub_frame->length){
        CODEC_PRINT("cmd_offset bigger than frame_size\n\n");
    ...
    while(pCmdData<pCmdEnd)
    {
        ...
        switch(spu_cmd)
        {
            ...
            case CMD_END:
                ...
                CODEC_PRINT("get_spu_cmd parser to the end\n\n");
                return 0;
                break;
            default:
                return -1;
    ...
    CODEC_PRINT("get_spu_cmd can not parser complete\n\n");
    return -1;
}
...
int get_vob_spu(char *spu_buf, unsigned length, AML_SPUVAR *spu)
{
	...
	while (spu->length-rd_oft > 0){
		if (!current_length) {
      		CODEC_PRINT("current_length is zero\n\n");
	...
	CODEC_PRINT("spu->frame_rdy is %d\n\n",spu->frame_rdy);
	if (spu->frame_rdy == 1){
		pixDataOdd = malloc(VOB_SUB_SIZE/2);
		CODEC_PRINT("pixDataOdd is %x\n\n",pixDataOdd);
		if (!pixDataOdd){
		CODEC_PRINT("pixDataOdd malloc fail\n\n");
		...
		CODEC_PRINT("pixDataEven is %x\n\n",pixDataEven);
		if (!pixDataEven){
      	CODEC_PRINT("pixDataEven malloc fail\n\n");
			goto error; 		//not enough memory
    }
...
error:
	if (pixDataOdd){
    	CODEC_PRINT("start free pixDataOdd\n\n");
		free(pixDataOdd);
		CODEC_PRINT("end free pixDataOdd\n\n");
	}
	if (pixDataEven){
    	CODEC_PRINT("start free pixDataEven\n\n");
		free(pixDataEven);
		CODEC_PRINT("end free pixDataEven\n\n");
	}
	return ret;
}
