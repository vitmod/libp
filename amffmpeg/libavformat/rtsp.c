    ...
    case 'a':
        if (av_strstart(p, "control:", &p)) {
            ...
            if (rt->server_type == RTSP_SERVER_WMS)
              ;//TOFIX---wms  ff_wms_parse_sdp_a_line(s, p);
            if (s->nb_streams > 0) {
...
{
    const char *p;
	av_log(NULL,AV_LOG_INFO,"ff_rtsp_parse_line: %s\n",buf);
    ...
    if (av_stristart(p, "Session:", &p)) {
        ...
        p += strspn(p, SPACE_CHARS);
      //TOFIX  ff_http_auth_handle_header(&rt->auth_state, "WWW-Authenticate", p);
    } else if (av_stristart(p, "Authentication-Info:", &p) && rt) {
        p += strspn(p, SPACE_CHARS);
       //TOFIX ff_http_auth_handle_header(&rt->auth_state, "Authentication-Info", p);
                       ...
                       int return_on_interleaved_data, const char *method)
{
    ...
    for (;;) {
        ...
        *q = '\0';
        ...
        av_log(s,AV_LOG_INFO,"ff_rtsp_read_reply,line='%s'\n", buf);
                                               ...
                                               int send_content_length)
{
    ...
    out_buf = buf;
    rt->seq++;
	av_log(NULL,AV_LOG_INFO, "headers=%s,url=%s\r\n",headers,url);
    ...
    if (rt->auth[0]) {
		av_log(s,AV_LOG_INFO,"rt->auth[0]---\n", rt->auth[0]);
			#if 0//TOFIX
    ...
    if (send_content_length > 0 && send_content) {
		...
		av_log(s, AV_LOG_INFO,"send_content:\n%s--\n", buf);
...
retry:
    ...
    if ((ret = ff_rtsp_send_cmd_with_content_async(s, method, url, header,
                                                   send_content,
                                                   send_content_length))){
        av_log(s, AV_LOG_INFO, "ff_rtsp_send_cmd_with_content_async %d\n", ret);                                           
        return ret;
    	}

    if ((ret = ff_rtsp_read_reply(s, reply, content_ptr, 0, method) ) < 0){
		av_log(s, AV_LOG_INFO, "ff_rtsp_read_reply %d\n", ret);    
        return ret;
    	}
...
redirect:
    ...
    ff_url_join(rt->control_uri, sizeof(rt->control_uri), "rtsp", NULL,
		...
		av_log(s, AV_LOG_INFO,"ff_rtsp_connect:[%d] rt->control_transport=%d\n",__LINE__,rt->control_transport);
#if 0
...
#else
	av_log(s, AV_LOG_INFO,"ff_rtsp_connect:[%d]\n",__LINE__);	
	 	ff_url_join(tcpname, sizeof(tcpname), "tcp", NULL, host, port, NULL);
		av_log(s, AV_LOG_INFO,"ff_url_join:[%s]--%d-[%s]\n",tcpname,port,host);
  
        if (url_open(&rt->rtsp_hd, tcpname, AVIO_FLAG_READ_WRITE) < 0) {
			av_log(s, AV_LOG_INFO,"url_open:[%s] failed\n",tcpname);
            err = AVERROR(EIO);
            goto fail;
        }
        rt->rtsp_hd_out = rt->rtsp_hd;
    ...
    for (rt->server_type = RTSP_SERVER_RTP;;) {
        ...
        if (reply->status_code != RTSP_STATUS_OK) {
			av_log(s, AV_LOG_INFO,"ff_rtsp_send_cmd:failed\n");
            err = AVERROR_INVALIDDATA;
            goto fail;
        }
